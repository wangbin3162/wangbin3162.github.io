...
> 目标：将其极简白底、大胆排版、滚动驱动的交互风格迁移到个人站点
...
| 角色 | 变量名 | HEX | 用途 |
|------|--------|-----|------|
| 主背景 | `--bg-primary` | `#ffffff` | 页面底色 |
| 辅背景 | `--bg-secondary` | `#f5f5f5` | 略带灰度的白，用于卡片/区块 |
| 强调色 | `--accent` | `#D14836` | 链接、编号、hover 状态 |
| 强调色亮 | `--accent-light` | `rgba(209,72,54,0.12)` | pill 背景、弱化强调 |
| 主文字 | `--text-primary` | `#000000` | 标题、正文 |
| 次文字 | `--text-secondary` | `rgba(0,0,0,0.6)` | 描述、辅助信息 |
| 弱文字 | `--text-muted` | `rgba(0,0,0,0.35)` | 日期、标签、编号括号 |
| 边框 | `--border` | `rgba(0,0,0,0.08)` | 分隔线、卡片边框 |
...
| text-fade | `linear-gradient(180deg, #000 0%, rgba(0,0,0,0.6) 100%)` | 大段文字渐隐 |
...
> 白底上阴影本身就很难体现，更多依赖 opacity 和 border 变化来区分层级。
...
导航栏：透明背景、黑色文字、左侧 logo + 右侧三到四个链接，无背景色变化。
...
过渡层为全屏遮罩，背景色 `#000000`（过渡专用，与页面背景形成对比）：

1. **离开动画**：当前内容 `opacity 0 → 1` 淡出（300ms）
2. **遮罩进场**：全屏遮罩从底部滑入覆盖全屏（600ms, ease-out）
